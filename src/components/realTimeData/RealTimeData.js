...
const RealTimeData = () => {
  ...
  useEffect(() => {
    ...
    const onDataReceived = (receivedData) => {
      ...
      const updatedEventDurations = EventDurationRealTime(
        receivedData,
        eventDuration
      )
      const newDuration = ConvertDataForScatterPlotRealTime(
        updatedEventDurations
      )
      ...
      const newData = Object.entries(updatedEventCounts).map(
        ([eventType, count]) => ({
          name: eventType,
          value: count,
        })
